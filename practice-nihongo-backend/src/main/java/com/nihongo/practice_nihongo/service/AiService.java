...
public class AiService {
    ...
    public String generateGrammarDetails(String structure, String existingSentence) throws Exception {
        try {
            ...
            String prompt = "You are a professional Japanese teacher. For the Japanese grammar structure provided, " +
                    ...
                    "  \"quizSentence\": \"(You MUST copy the exampleSentence exactly, but replace the exact conjugated grammar word/phrase with '_____'. CRITICAL: DO NOT remove the verbs, nouns, or particles preceding the grammar point! Only replace the grammar structure itself. Example: if grammar is '〜ことになっている' and sentence is '挨拶をすることになっている。', output '挨拶をする_____。' (do NOT remove the verb 'する'). Even if the grammar changes form, only replace the grammar part. There MUST be exactly one '_____' per sentence. Separate multiple sentences by \\n.)\"\n" +
                    "}\n" +
    ...
    private String buildPrompt(String rawData, String type) {
        ...
        return "You are a professional Japanese data formatter. Convert the following messy data into a valid JSON array strictly following this schema: " + schema + 
               ...
               "6. For Grammar, the \"quizSentence\" MUST be exactly the \"exampleSentence\", but you MUST replace the conjugated grammar point with '_____'. CRITICAL: DO NOT remove the verbs, nouns, or particles preceding the grammar point! Only replace the grammar structure itself. Example: if grammar is '〜ことになっている' and sentence is '挨拶をすることになっている。', output '挨拶をする_____。' (do NOT remove the verb 'する'). Even if the grammar changes form (e.g., に囲まれて instead of に囲まれる), you MUST find that part in the sentence and replace it with '_____'. There MUST be exactly one '_____' in the string.\n" +
