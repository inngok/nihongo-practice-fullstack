...
public class AiService {
    ...
    public String generateGrammarDetails(String structure, String existingSentence) throws Exception {
        try {
            ...
            String prompt = "You are a professional Japanese teacher and linguistics expert. For the Japanese grammar structure provided, " +
                    ...
                    "  \"explanation\": \"(CRITICAL: Put the conjugation formula exactly starting with 'Cấu trúc: \\n'. MUST ALWAYS format the formula using this exact custom block syntax: \\n[[\\n(List the accurate conjugations for V, Aい, Aな, N)\\n||\\n+ (grammar suffix)\\n]]\\nExample:\\n[[\\nV普通形\\nAい\\nAな (+な)\\nN (+の)\\n||\\n+ ようだ\\n]]\\nCRITICAL RULE: You MUST deduce the correct grammatical rules for the structure EVEN IF the user provided no rules! If a verb drops 'ます' or a Na-adjective drops 'な', you MUST explicitly indicate this dropping in Vietnamese using parentheses. Examples: 'Vます (bỏ ます)', 'Aい (bỏ い)', 'Aな (bỏ な)'. DO NOT use strikethrough (like ~~な~~). If a Na-adjective or Noun attaches directly without 'な' or 'だ' (like for というものだ), write 'Aな (bỏ な)' and 'N (bỏ だ)'. Do NOT blindly copy (+な) or (+だ) if it is grammatically incorrect for the current structure. The left side (before ||) MUST list the word types separated by newlines. The right side (after ||) MUST be the grammar suffix. YOU MUST ALWAYS use this [[ left || right ]] block format, even if there is only one word type on the left. The grammar suffix MUST be placed INSIDE the block after ||, NOT outside. MUST use Japanese terminology (e.g., V普通形, Vます (bỏ ます), Aい, Aな (bỏ な), N). Then add an extra newline \\n, and write the explanation starting with 'Giải thích: '. KEEP THE EXPLANATION EXTREMELY SHORT, MAXIMUM 2 SENTENCES!)\",\n" +
    ...
    private String buildPrompt(String rawData, String type) {
        ...
        return "You are a professional Japanese data formatter and linguistics expert. Convert the following messy data into a valid JSON array strictly following this schema: " + schema +
               ...
               "8. For Kanji, the \"examples\" field contains vocabulary entries formatted as: \"Word (Reading): Vietnamese meaning\". Separate each entry with a semicolon (;). Example: \"地形 (ちけい): địa hình; 形成 (けいせい): hình thành;\". If the user DOES NOT provide any vocabulary, leave the \"examples\" field empty. DO NOT auto-generate vocabulary.\n" +
               "9. CRITICAL FOR GRAMMAR: The \"structure\" field MUST ONLY contain the Japanese grammar point using a tilde '〜' (e.g. '〜に囲まれている', '〜を占める'). DO NOT include conjugation formulas (like 'N +' or 'V-ru +') in the \"structure\" field. All conjugation formulas MUST be placed at the very beginning of the \"explanation\" field, starting with 'Cấu trúc: \\n'. MUST ALWAYS format the formula using this exact custom block syntax: \\n[[\\n(List the accurate conjugations for V, Aい, Aな, N)\\n||\\n+ (grammar suffix)\\n]]\\nExample:\\n[[\\nV普通形\\nAい\\nAな (+な)\\nN (+の)\\n||\\n+ ようだ\\n]]\\nCRITICAL RULE: You MUST deduce the correct grammatical rules for the structure EVEN IF the user provided no rules! If a verb drops 'ます' or a Na-adjective drops 'な', you MUST explicitly indicate this dropping in Vietnamese using parentheses. Examples: 'Vます (bỏ ます)', 'Aい (bỏ い)', 'Aな (bỏ な)'. DO NOT use strikethrough (like ~~な~~). If a Na-adjective or Noun attaches directly without 'な' or 'だ' (like for というものだ), write 'Aな (bỏ な)' and 'N (bỏ だ)'. Do NOT blindly copy (+な) or (+だ) if it is grammatically incorrect for the current structure. The left side (before ||) MUST list the word types separated by newlines. The right side (after ||) MUST be the grammar suffix. CRITICAL: The grammar suffix on the right side MUST EXACTLY MATCH the grammar point string (minus the '〜') shown in the 'structure' field! DO NOT aggressively split or reduce the grammar structure. For example, if the structure is '〜ないことは（も）ない', the right side MUST be '+ ないことは（も）ない' (do NOT reduce it to '+ ことはない' by moving 'ない' to the left). YOU MUST ALWAYS use this [[ left || right ]] block format, even if there is only one word type on the left. The grammar suffix MUST be placed INSIDE the block after ||, NOT outside. You MUST use Japanese terminology (e.g., V普通形, Vます (bỏ ます), Aい, Aな (bỏ な), N). Then, YOU MUST insert an extra newline character (\\n), and start the actual usage explanation with 'Giải thích: '. KEEP THE EXPLANATION EXTREMELY SHORT, MAXIMUM 2 SENTENCES!\n" +
               ...
               "13. KANJI EXAMPLES: Only use the vocabulary words explicitly provided by the user in the input. DO NOT generate supplementary or extra examples if the user did not provide them.\n" +
               ...
               "17. CRITICAL FOR GRAMMAR - PRESERVE AND CORRECT USER DATA: If the input data provides a Vietnamese meaning or a conjugation formula (e.g., 'Cấu trúc: V/A/N + ても／なくても'), you MUST convert their formula into the required [[ || ]] block format (from Rule 9). HOWEVER, you MUST aggressively check the academic accuracy of their conjugation formula. If their formula is imprecise or missing critical conjugation details (e.g., they wrote 'V' or 'Vます' but the grammar actually requires dropping masu, or they wrote 'N' but it requires 'Nの'), you MUST CORRECT IT to be academically perfect, applying the explicit '(bỏ...)' rule from Rule 9. For the 'meaning' field, keep their core intent, but correct it if it is wrong.\n" +
               "18. CRITICAL FOR GRAMMAR - DO NOT SPLIT SYNONYMOUS PATTERNS: If an input grammar item contains multiple synonymous patterns separated by slashes (/) or dots (・) (e.g., '〜をきっかけに / 〜がきっかけで'), DO NOT split them into separate JSON objects. Keep them together as a SINGLE object where the \"structure\" field contains the full combined string. However, if two grammar items have the same name but different numbers/meanings (e.g. '〜どころか (1)' and '〜どころか (2)'), you MUST process them as TWO SEPARATE JSON objects.";
    }
