...
export default function UserManager() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button type="text" className="dark:text-slate-400 dark:hover:text-white flex items-center justify-center" icon={<EditOutlined />} onClick={() => handleEdit(record)} />
          <Popconfirm
    ...
    <div className="flex-grow w-full py-8 px-10 animate-in fade-in duration-500 text-slate-900 dark:text-slate-100 bg-transparent">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-12">
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Quản lý User</h1>
            <p className="text-slate-400 dark:text-slate-500 text-[13px] font-medium">Quản lý tài khoản và phân quyền hệ thống</p>
          </div>
          <button
            onClick={handleAdd}
        ...
        <div className="border border-slate-100 dark:border-slate-800 rounded-xl overflow-hidden premium-table bg-white dark:bg-slate-900/50">
          <Table 
            columns={columns} 
            dataSource={users} 
            rowKey="id"
