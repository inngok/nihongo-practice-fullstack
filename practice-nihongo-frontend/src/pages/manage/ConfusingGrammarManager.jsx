...
export default function ConfusingGrammarManager() {
  ...
  const handleTriggerAiChatGenerate = async () => {
    ...
    try {
      ...
      // Update the pattern input tags list state to keep it in sync
  ...
  const handleSave = async (values) => {
    // If items were not filled (neither manually nor by AI)
    const validPatterns = patternInputs.filter(p => p.trim() !== '');
    
    // Create payload structure matching backend
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Right side: Generated or filled fields */}
          ...
          {/* Form List representing point-level details generated by AI */}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            <Button onClick={() => setIsOpen(false)} className="rounded-xl font-bold h-10">
              Hủy
            </Button>
            <Button
              type="primary"
