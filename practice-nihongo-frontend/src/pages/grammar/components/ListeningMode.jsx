...
import { ChevronLeft, ChevronRight, ArrowLeft, Volume2, Play } from 'lucide-react';
...
export default function ListeningMode({
  ...
  const playExpressiveAudio = async () => {
    const text = activeData[currentIndex]?.quiz?.sentence;
    if (!text) return;

    setIsPlaying(true);
    try {
      const response = await fetch(`https://api.tts.quest/v3/voicevox/synthesis?text=${encodeURIComponent(text)}&speaker=2`);
      const data = await response.json();

      if (data.isApiKeyValid !== false && data.mp3StreamingUrl) {
        const audio = new Audio(data.mp3StreamingUrl);
        audio.onended = () => setIsPlaying(false);
        audio.onerror = () => fallbackAudio(text);
        audio.play();
        return;
      }
    } catch (e) {
      console.error('Voicevox fetch error, falling back to Web Speech API', e);
    }
    fallbackAudio(text);
  };

  const fallbackAudio = (text) => {
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel();
      const utterance = new SpeechSynthesisUtterance(text);
      utterance.lang = 'ja-JP';
      const voices = window.speechSynthesis.getVoices();
      const bestVoice = voices.find(v => v.lang.includes('ja') && (v.name.includes('Natural') || v.name.includes('Premium'))) || voices.find(v => v.lang === 'ja-JP');
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-2xl mx-auto pb-24 sm:pb-0">
      ...
      <div className="bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-8 sm:p-12 shadow-sm text-center relative overflow-hidden">
        ...
        <span className="px-4 py-1.5 bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 rounded-full text-[9px] font-black uppercase tracking-wider mb-6 inline-block border border-slate-100 dark:border-slate-900">
          NGHE VÀ ĐIỀN NGỮ PHÁP
        </span>

        <div className="flex justify-center mb-6">
          <button
            onClick={playExpressiveAudio}
            disabled={isPlaying}
            className={`w-16 h-16 flex items-center justify-center rounded-full transition-all duration-300 ${isPlaying ? 'bg-slate-100 dark:bg-slate-800 text-slate-400' : 'bg-black text-white hover:scale-105 hover:shadow-lg dark:bg-white dark:text-black'}`}
          >
